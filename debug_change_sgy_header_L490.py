...
Updated Mar. 2023 - Test on other lines with issues
...
# Debug line 125
for j, file in enumerate(files[18:19]):  # this is line 128a
    ...
    ffids, traces = np.zeros([n]), np.zeros([n])
    for i in range(n):
        ffid = (h[i][byte_ffid])
        ffids[i] = ffid
        ...
        if nav_trace.size > 0:
            ...
            easting, northing = remove_decimal(coords[0])[0], remove_decimal(coords[1])[0]
            ...
            # Take the mean of the shot location before and after the missing data
            nav_trace = nav_line[(nav_line.shot <= ffid+1) & (nav_line.shot >= ffid-1)]
...
#         if err_mask[i] == True:
#             coords = utm.from_latlon(lat, lon, zone)
